...
const GET_EMBEDDER_QUERY: &str = r#"
    SELECT embedder_id, name, owner, provider, base_url, api_key, config, batch_size, collection_name, created_at, updated_at 
    FROM embedders
...
const CREATE_EMBEDDER_QUERY: &str = r#"
    INSERT INTO embedders (name, owner, provider, base_url, api_key, config, batch_size, collection_name, created_at, updated_at)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())
    RETURNING embedder_id, name, owner, provider, base_url, api_key, config, batch_size, collection_name, created_at, updated_at
"#;
...
const UPDATE_EMBEDDER_QUERY: &str = r#"
    ...
    SET name = COALESCE($3, name),
        ...
        collection_name = COALESCE($8, collection_name),
        updated_at = NOW()
    WHERE owner = $1 AND embedder_id = $2
    RETURNING embedder_id, name, owner, provider, base_url, api_key, config, batch_size, collection_name, created_at, updated_at
"#;
