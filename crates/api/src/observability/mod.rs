...
pub(crate) fn init_observability() -> Result<PrometheusMetrics> {
    ...
    match trace_exporter {
        ...
        Err(e) => {
            tracing::warn!(
                "Failed to initialize OpenTelemetry tracer (set OTEL_EXPORTER_OTLP_ENDPOINT to enable): {e:?}"
            );
        }
    }
    ...
    let logger_provider = match log_exporter {
        Ok(exporter) => {
            let provider = SdkLoggerProvider::builder()
                ...
                .with_resource(resource.clone())
                .build();
            tracing::info!("OpenTelemetry logger initialized successfully");
            Some(provider)
        }
        Err(e) => {
            tracing::warn!("Failed to initialize OpenTelemetry logger: {e:?}");
            None
        }
    };

    // There's an existing issue where it will log warnings but will not capture the metrics properly.
