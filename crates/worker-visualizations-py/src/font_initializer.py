...
def disable_datamapplot_font_requests() -> None:
    ...
    try:
        ...
        def mock_can_reach_google_fonts(_timeout: float = 5.0) -> bool:
            ...
            logger.debug("can_reach_google_fonts called - returning False (offline mode)")
            return False
        ...
        def mock_query_google_fonts(fontname):
            ...
            logger.debug(f"query_google_fonts called for '{fontname}' - returning empty (offline mode)")
        ...
        logger.info("Successfully disabled datamapplot runtime font requests (offline mode enabled)")
