    ...
    try:
        ...
        for font_file in font_files:
            try:
                ...
                # Determine MIME type
                mime_type = "font/woff2" if font_file.suffix == ".woff2" else "font/woff"
        ...
        logger.info(f"Loaded and embedded {embedded_count} font files ({len(css_content)} bytes CSS)")
...
def patch_html_fonts(html_content: str) -> str:
    ...
    try:
        ...
        if not local_font_css:
            logger.warning("No local fonts available, removing Google Fonts links to avoid SSL errors")
